#pragma once // PREPROCESSOR WRAPPER USED FOR PREVENT CLASS DEFITION SEVERAL TIMES 
...
class DeckQueue {
  ...
  int value; // PUBLIC DATA MEMBER
   ...
   int valid_input (); // MEMBER FUNCTION THAT VALIDATE INPUT VALUE
   void display () const; // MEMBER FUNCTIONT THAT DISPLAY DECKQUEUE 
   DeckQueue& insert_end (const int&); // MEMBER FUNCTION THAT INSER VALUE AT THE END OF THE QUEUE
   DeckQueue& insert_front (const int&); // MEMBER FUNCTION THAT INSERT VALUE AT THE FRONT OF THE QUEUE
   ...
   inline bool isQueueEmpty () const; // MEMBER METHOD THAT CHECKS QUEUE IS EMPTY OR NOT
   inline bool isQueueFulled () const; // MEMBER METHOD THAT CHECKS QUEUE IS FULLED OR NOT 
}; // END OF THE CLASS 
